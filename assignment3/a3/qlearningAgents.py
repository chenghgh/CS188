...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        '''
        Using random.choice function to help
        '''
        legalActions = self.getLegalActions(state)
        if not legalActions:
            return None
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        weight = weight + alpha * diff * f(s,a)
        '''
        # print(self.featExtractor.getFeatures(state, action))
        diff = reward + self.discount * self.getValue(nextState) - self.getQValue(state,action)
        # self.weights = self.weights +  self.alpha * diff * self.featExtractor.getFeatures(state, action)
        for i in self.featExtractor.getFeatures(state, action):
            self.weights[i] += self.alpha * diff * self.featExtractor.getFeatures(state, action)[i]
